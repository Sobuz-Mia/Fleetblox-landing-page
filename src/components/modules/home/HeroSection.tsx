...
const HeroSection = () => {
  ...
  return (
    <section className="bg-[#FAFAFF] flex flex-col justify-center items-center z-50 overflow-hidden">
      {/* LCP content prioritized */}
      <div className="mt-[120px] pb-3 flex flex-col items-center justify-center px-5 z-30 relative">
        <div className="max-w-[840px] w-full mx-auto text-center flex flex-col items-start md:items-center">
          ...
          <h3 className="text-[36px] md:text-[52px] lg:text-[52px] text-left md:text-center font-bold text-[#04082C] leading-[1.1] font-montserrat z-30">
        ...
        {/* CTA Button */}
        <Link href="/getting-started" aria-label="Get started with FleetBlox">
          <button className="hidden md:flex transition-all font-openSans bg-[#2D65F2] hover:bg-[#0336BC] text-white duration-300 hover:w-[144.16px] w-[122.16px] items-center px-[13px] hover:px-4 py-3 text-base font-bold rounded-md group">
      ...
      <div className="hidden lg:block relative max-h-[800px] lg:h-[800px] md:h-[500px] lg:w-[1200px] xl:w-[1400px] z-0 overflow-hidden">
        <div className="relative h-full w-full flex justify-center items-center">
          ...
          <div className="max-h-[400px] h-full filter blur-sm absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20 rounded-[520px] max-w-[520px] w-full bg-[#2D65F2]"></div>
        ...
        <div className="absolute -bottom-[10px] z-50 left-1/2 max-w-[880px] w-full gap-5 -translate-x-1/2 py-[30px] grid grid-cols-1 lg:grid-cols-3 text-center items-start justify-items-center">
          {features.map((feature, index, arr) => (
            <div key={feature.title} className="flex items-center justify-center gap-5">
              <div className="text-center">
                <h4 className="text-gray-900 font-bold text-[18px] font-openSans">{feature.title}</h4>
                <p className="text-gray-700 text-[14px] leading-5 font-openSans">{feature.description}</p>
              </div>
      ...
      <div className="lg:hidden mt-8 relative w-full flex flex-col items-center justify-center">
        ...
        <div className="px-4 pt-10 pb-6 space-y-6">
          {features.map((feature, index) => (
            <div key={index} className="flex flex-col pb-4 items-center last:border-0">
